...
# Banco de dados expandido para evitar repetição visual
IMAGENS_BACKUP = [
...
<style>
    ...
    .img-container { width: 100%; aspect-ratio: 16/9; background: #f8f8f8; border-radius: 8px; overflow: hidden; margin-bottom: 12px; border: 1px solid #eee; }
    ...
    .noticia-body p { font-size: 0.85em; color: #555; margin: 10px 0; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; line-height: 1.4; }
...
def limpar_html(texto):
    """Remove tags HTML que vazam para o texto do card"""
    if not texto: return ""
...
def processar_noticia_ai(titulo, resumo, categoria):
    ...
    prompt = f"Analista G1. Cat: {categoria}. Titulo: {titulo}. Resumo: {resumo_limpo}. Responda apenas um JSON puro: {{\"manchete\": \"string\", \"materia\": \"string 3 parágrafos\", \"sentimento\": \"Positivo|Negativo|Neutro\"}}"
    try:
...
def capturar_imagem(entry):
    """Tenta extrair imagem real do feed, senão usa backup aleatório"""
    if 'media_content' in entry: return entry.media_content[0]['url']
    if 'media_thumbnail' in entry: return entry.media_thumbnail[0]['url']
    # Busca no conteúdo caso esteja escondida
    if 'summary' in entry:
...
def gerar_pagina_individual(id_noticia, data, cat, img):
    ...
    html_content = f"<!DOCTYPE html><html><head><meta charset='UTF-8'><title>{m_esc}</title>{CSS}</head><body style='padding:20px;'><header><a href='../index.html'>← VOLTAR</a></header><article><h1>{m_esc}</h1><img src='{img}' style='width:100%; border-radius:12px;'><p>{mat_esc}</p></article></body></html>"
...
def extrair_noticias(item):
    ...
    for entry in feed.entries[:3]:
        ...
        m_esc = html.escape(data['manchete'])
        mat_preview = limpar_html(data['materia'])[:130]
...
def atualizar_portal():
    ...
    final = f"<!DOCTYPE html><html lang='pt-BR'><head><meta charset='UTF-8'><title>Portal IA News</title>{CSS}</head><body><header><div class='weather-widget' id='weather-display' onclick='alterarCidade(event)'>⏳</div><h1>Portal IA News</h1></header>{filtros}<div class='main-wrapper'><div class='content-area'>{novas}{antigas}</div><div class='sidebar'><h3>Histórico</h3><div class='sidebar-list'>{novos_h}{h_antigo}</div></div></div>{JS}</body></html>"
